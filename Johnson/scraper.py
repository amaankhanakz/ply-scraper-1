...
url_pages='https://www.johnson-tiles.com/catalog/search/categories/concrete/'
file="concrete"
file_name=file+".csv"

csv_file = open(file_name, 'w', newline='', encoding="utf-8")
csv_writer = csv.writer(csv_file)
csv_writer.writerow(['url', 'name', 'Recycle % ', 'Range', 'Short Code',"Colour ","Finish","Rectified Edge",'Wall Suitability'
, 'Floor Suitability',"Wet Barefoot","Material","Classification","Light Reflectance Value", "CSV","PTV (4S)","PTV (TRL)",
'R Value','Wet Ramp',"PEI","Traffic","Grout Suggestions","Size","Size With Full Product Code","Size images"
...
try :
    pages_finding=soup_pages.find("ul",class_="products__pagination-list").find_all("li")
    total_pages=int(pages_finding[-2].find("a").text)

except Exception as e:
    try :
        pages_finding=pages_finding=soup_pages.find("ul",class_="products__pagination-list").find_all("li")
        total_pages=int(pages_finding[-3].find("a").text)
    except :
        total_pages=1
...
for i in range (1,total_pages+1):
    ...
    for item in all_items_product_div:
        ...
        try :
            product_name=soup_single_item.find("div",class_="product-header").find("h1",class_="product-header__info__name").text
        except:
            product_name=""
        ...
        try :
            ...
            product_img=""
        try:
            product_recycle=soup_single_item.find("div",class_="product__stats__eco-friendly").find("p",class_="value").text
        except:
            product_recycle=""
        ...
        csv_writer.writerow([product_url, product_name,product_recycle, product_range,product_short_code, product_colour,product_finish , product_rectified_edge
        ,product_wall_suitability,product_floor_suitability,product_wet_barefoot,product_material,product_classification,product_light_reflectance_value
        ,product_CSV,product_PTV_4S,product_PTV_TRL,product_R_value,product_wet_ramp,product_PEI,product_traffic,product_grout_suggestions
        ,product_size,product_size_code,product_size_image,product_fitting,product_fitting_code,product_img])
